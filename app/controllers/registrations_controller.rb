class RegistrationsController < Devise::RegistrationsController
  ...
  def create
    ...
    if resource.save
      ...
      if !user.nil? && user.send_confirmation_instructions
        render_json_message(:ok, message: 'Account Pending; please check your email to confirm your account!', to: about_path)
      else
  ...
  def configure_permitted_parameters
    devise_parameter_sanitizer.for(:sign_up) do |u|
      u.permit(:is_admin, :first_name, :last_name, :organization, :location_id, :on_map,
               :email, :password, :password_confirmation, :profile_pic, :website,
               :secondary_name, :secondary_email, :tertiary_name, :tertiary_email,
               :volunteer, :seeking_volunteer)
    end
  end
